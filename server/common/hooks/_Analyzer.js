...
import { getEthHistoricalPrice, getShitcoinHistoricalPrice, getShitcoinPrice } from './_crypto.js'
...
export default class Analyzer {
  ...
  count_avg_time(txs, symbol, isTuple = false) {
    /*
        ...
        */
    const first_buy_ts = txs.filter(
      (tx) => tx._to === this.address && JSON.parse(tx.token_info).symbol === symbol
    ) // this is all buys
    const last_sell_ts = txs.filter(
      (tx) => tx._from === this.address && JSON.parse(tx.token_info).symbol === symbol
    ) // all sells

    // no sells for token or buys
    if (!last_sell_ts.length || !first_buy_ts.length) return 0
    return (
      Number(last_sell_ts[0].created_at) - Number(first_buy_ts[first_buy_ts.length - 1].created_at)
    )
  }
  ...
  define_message(isHoneypot, realizedUsd, unrealizedUsd, buy) {
    ...
    else if (realizedUsd + unrealizedUsd == 0 && buy != 0) return 'TOKENS TRANSFERRED'
    return
  }
    ...
    for (let tx of txs) {
      ...
      db_txs.push([
        ...
        JSON.stringify({
          ...
          honeypot: data[tx.tokenSymbol]['honeypot']
        }),
        gas_usd
      ])
    }
  ...
  async get_wallet_tokens() {
    ...
    const data = {
      ...
      params: [this.address, 'erc20']
    }
    return await new Promise((resolve, reject) => {
      axios
        .post(config.HTTPS_PROVIDER, data, {
          headers: headers
        })
        .then((res) =>
          res.data?.error || res.status !== 200
            ? reject([])
            : resolve(res.data.result?.tokenBalances)
        )
    })
  }
  ...
  async parse_info(first_calculation = false) {
    ...
    for (let token of Object.keys(prices)) {
      ...
      table_data['tokens_info'].push({
        ...
        fees: fees,
        message: this.define_message(
          prices[token]['honeypot'],
          sell,
          prices[token]['unrealized_$'],
          buy
        )
      })
    }
    ...
    if (table_data['tokens_info'].length == 0) return
    ...
    table_data['general_info']['pnl_$'] = table_data['tokens_info']
      .reduce((total, el) => total + el['total_$'], 0)
      .toFixed(2) // sum of array of total $ of each token
